...
本技能可以独立完成候选评估、入场判断、退出计划和案例复盘。`plan-review` 报告是可选参考；本技能必须自行核验所需的原始市场数据。补充渠道取得的数据只有在日期、时点、范围、单位和来源均核验通过后才能进入决策。
...
不得只给总分。若使用评分，必须同时展示构成证据、反证和未知项。决策完成后必须将标准化报告保存至用户当前工作空间的 `./report/strategy/YYYYMMDD_首板隔夜决策.md`（带标准 Frontmatter，参见 [`templates/strategy_report.md`](templates/strategy_report.md)），若目录不存在则自动创建，切勿写入技能代码安装目录。
